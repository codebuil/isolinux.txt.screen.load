...
void prints(char *c)
{
 ...
 void printcc(char b,char colorss)
        {
        		   int i=video;
			   char *fbp=(char* )i;
			   if (b==10 || b==13)
			   {
			   	...
			   	*((char *)(fbp)) =(char)b;
			   	*((char *)(fbp+1)) =(char)colorss;
			   video++;
			   video++;
			   }  
        }
void printss(char *c,int x, int y, int w,int h)
{
	int counter=0;
	int n=0;
	
	for (n=0;n<h;n++){
		video=0xb8000+(((y+n)*160)+x*2);
		video2=video;	
		while(c[counter]!=13){
			if(c[counter]!=13){
				printcc(c[counter],c[counter+1]);
				counter++;
				counter++;
			}
		}
